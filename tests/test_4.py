...
@pytest.fixture
def setup_tokens():
    # Генерация Ed25519 ключей для v4
    ...
    tokens = {
        'local': {
            'v4': PasetoToken(key=local_key, version='v4', purpose='local', payload=local_payload),
        },
        'public': {
            'v4': PasetoToken(key=private_key, version='v4', purpose='public', payload=public_payload),
        },
        'verify': {
            'v4': public_bytes
        }
